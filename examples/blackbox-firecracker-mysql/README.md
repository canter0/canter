...
> Live result on 2026-08-28: the selected `c1` substrate did not expose `/dev/kvm`. The host rejected the deployment before downloading artifacts, Canter recorded a failed proof, and the host was destroyed. The generated code passed structural acceptance, but the two live MySQL guests remain unverified on this compute class. See [`docs/firecracker-acceptance.md`](../../docs/firecracker-acceptance.md).
