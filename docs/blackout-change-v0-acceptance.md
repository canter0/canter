...
Network identifiers and infrastructure endpoints in this historical record
have been replaced with documentation-safe placeholders. Execution timing,
state, and failure semantics are unchanged.
...
```text
canter: Put "https://compute-provider.invalid/network/ports/c8441add-7115-4095-85f5-07f8f11b0bf5": context deadline exceeded (Client.Timeout exceeded while awaiting headers)
```
...
Result: one ACTIVE resource `45f5fd45-bb1c-4d3d-92f4-b8d36b941330` at `203.0.113.20`; release phase `waiting`; private `database/postgres` phase `ready`, endpoint `127.0.0.1:5432`.
...
```sh
curl --noproxy '*' -fsS http://203.0.113.20:8080/health
```
...
```sh
curl --noproxy '*' -fsS http://203.0.113.20:8080/availability
curl --noproxy '*' -fsS -X POST -H 'Content-Type: application/json' \
  -d '{"id":"baseline-neighbor"}' http://203.0.113.20:8080/accounts
curl --noproxy '*' -fsS -X POST http://203.0.113.20:8080/claims/baseline-neighbor
curl --noproxy '*' -fsS http://203.0.113.20:8080/proof
```
...
```sh
curl --noproxy '*' -fsS http://203.0.113.20:8080/change-proof
curl --noproxy '*' -fsS http://203.0.113.20:8080/availability
```
...
```text
canter: delete network policy 09616c6d-6e5d-43fa-a41a-be8adef271b6: Delete "https://compute-provider.invalid/network/security-groups/09616c6d-6e5d-43fa-a41a-be8adef271b6": context deadline exceeded (Client.Timeout exceeded while awaiting headers)
```
