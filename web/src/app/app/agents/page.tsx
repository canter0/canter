...
export default function AgentsPage() {
  ...
  const installations = data?.installations.filter(agent => agent.harness !== "canter-hosted" && agentIsConnected(agent)) ?? [];

  return <AppShell active="Agents"><section className={styles.contentPage}>
    <div className={styles.pageHeading}><div><h1>Your agents</h1><p>Bring the agents you already work with.</p></div><div className={styles.pageActions}><Link className={styles.secondaryButton} href="/app/settings/agent">Agent settings</Link><ConnectAgentButton className={styles.primaryButton}><WorkspaceIcon name="plus" width="16" height="16" />Connect agent</ConnectAgentButton></div></div>
