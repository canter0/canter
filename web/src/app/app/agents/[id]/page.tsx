...
import { agentIsConnected, authorityLabel, canterFetch, relativeTime, type Installation, type Me } from "@/lib/canter-api";
...
export default function AgentInstallationPage() {
  ...
  const connection = installation ? [
    ...
    ["authorization", agentIsConnected(installation) ? "authorized" : "disconnected"],
  ] : [];
  ...
  return (
    <AppShell active="Agents" context={`canter / default / ${installation?.name ?? "agent"}`}>
      <section className="flex min-h-[calc(100vh-80px)] flex-col overflow-x-auto px-6 pt-10 sm:px-10 lg:px-14 lg:pt-11">
        <div className="grid min-w-[720px] grid-cols-[1fr_220px] items-end border-b border-[var(--rule-strong)] pb-7"><div><div className="meta">Agent installation</div><div className="flex items-center gap-4"><h1 className="display mt-3 text-[32px]">{installation?.name ?? "Loading"}</h1><span className="mt-3 flex items-center gap-2 text-[10px]"><span className={`signal ${installation && !agentIsConnected(installation) ? "opacity-20" : ""}`} />{installation && !agentIsConnected(installation) ? "DISCONNECTED" : "CONNECTED"}</span></div></div><div className="pb-1 text-right text-[var(--muted)]">{installation ? `principal ${installation.id}` : null}</div></div>
        ...
        <div className="mt-12 min-w-[720px]"><SectionHeader left="CONTINUITY" right="CANTER IS THE RECORD" /><div className="border-b border-[var(--rule)] py-6 leading-5 text-[var(--muted)]">{installation?.expiresAt ? "This temporary connection ends when its task finishes, or after 8 hours. Its task history and activity remain available." : "This agent can reconnect until its access is revoked. Task history and activity remain available."}</div></div>
        <div className="mt-auto flex min-h-20 min-w-[720px] items-center justify-end border-t border-[var(--rule-strong)]"><button onClick={() => void revoke()} disabled={pending || !installation || !workspaceId || Boolean(installation.revokedAt)} className="flex h-10 w-[154px] items-center justify-center border border-[var(--rule-strong)] disabled:opacity-40">{pending ? "Revoking…" : "Revoke access"}</button></div>
      </section>
    </AppShell>
  );
}
