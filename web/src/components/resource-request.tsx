...
export function ResourceRequest({ kind, busy, onContinue, onGitHub }: { kind: ResourceKind; busy: boolean; onContinue: (message: string) => Promise<boolean>; onGitHub: () => void }) {
  ...
  async function submit(event: FormEvent) {
    ...
    try {
      const message = compute
        ? `Help me plan a VPS named ${name.trim()}. Intended workload: ${purpose.trim()}. Starting size preference: ${size}. Respect that I want a VPS, prepare a useful plan, and label assumptions. Check actual Canter capabilities; this is not authorization to provision or incur charges.`
  ...
  return <section className={styles.card} aria-label={compute ? "VPS setup" : "Storage bucket setup"}>
    ...
    <form onSubmit={event => void submit(event)}>
      ...
      <p className={styles.limit}>{compute ? "Canter can help plan standalone VMs and multi-VM setups in chat. Live provider availability, exact specifications, pricing, and standalone provisioning aren’t connected here yet." : "Standalone bucket creation isn’t available yet. These details help plan your request; no bucket or access permissions will be created."}</p>
