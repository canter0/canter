  ...
  return (
    <AppShell active="System" context={`canter / default / ${system} / policies`}>
      <section className="flex min-h-[calc(100vh-80px)] flex-col overflow-x-auto px-6 pt-10 sm:px-10 lg:px-14 lg:pt-11">
        <div className="grid min-w-[760px] items-end border-b border-[var(--rule-strong)] pb-7 lg:grid-cols-[1fr_170px_170px]">
        ...
        </div> : <div className="mt-14 min-w-[760px] border-t border-[var(--rule-strong)] py-8"><span className="text-[var(--muted)]">Policies begin from a real server-generated Change envelope.</span> <Link href="/app/changes" className="rule-link ml-2">Choose a Change ↗</Link></div>}
        <div className="mt-auto flex min-h-20 min-w-[760px] items-center justify-between border-t border-[var(--rule-strong)]"><Link href={`/app/system/${encodeURIComponent(system)}`} className="rule-link">Back to System ↗</Link>{change && envelope ? <button onClick={() => void createPolicy()} disabled={pending || !name || !installationId} className="flex h-10 min-w-[210px] items-center justify-center bg-[var(--ink)] px-6 text-[var(--paper)] disabled:opacity-30">{pending ? "Creating…" : "Create exact envelope"}</button> : null}</div>
      </section>
    </AppShell>
  );
}
