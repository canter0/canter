  ...
  return (
    <AppShell active="Changes" context={`canter / default / ${id}`}>
      <section className="flex min-h-[calc(100vh-80px)] flex-col overflow-x-auto px-6 pt-10 sm:px-10 lg:px-14 lg:pt-11">
        <div className="grid min-w-[740px] grid-cols-[1fr_210px] items-end border-b border-[var(--rule-strong)] pb-7"><div><div className="meta">{id} · drafted by {change?.draftedBy?.displayName ?? change?.draftedBy?.id ?? "agent"}</div><h1 className="display mt-3 text-[32px]">{change?.summary ?? "Loading Change"}</h1></div><div className="flex items-center justify-end gap-2 pb-1 text-[10px]"><span className="signal" />{change?.phase.toUpperCase() ?? "LOADING"}</div></div>
        ...
        <div className="mt-auto flex min-h-20 min-w-[740px] items-center justify-between gap-3 border-t border-[var(--rule-strong)]">
