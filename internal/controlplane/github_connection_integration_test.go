...
func TestGitHubAppOAuthRepositoryAccessAndIsolation(t *testing.T) {
	s, c, human := operatorFixture(t)
	ctx := context.Background()
	h := NewHTTPServer(&Service{Store: s}, HTTPConfig{PublicURL: "http://canter.test", GitHubApp: OAuthCredentials{ClientID: "client", ClientSecret: "secret"}}).(*HTTPServer)
	var challenge string
	provider := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		_ = r.ParseForm()
		if r.Form.Get("client_secret") != "secret" || r.Form.Get("redirect_uri") != "http://canter.test/api/canter/auth/oauth/github-app/callback" || oauth2.S256ChallengeFromVerifier(r.Form.Get("code_verifier")) != challenge {
			t.Error("repository OAuth lost PKCE or callback binding")
		}
		w.Header().Set("Content-Type", "application/json")
		fmt.Fprint(w, `{"access_token":"private-repository-token","token_type":"bearer","scope":"","expires_in":28800,"refresh_token":"refresh","refresh_token_expires_in":15897600}`)
	}))
	defer provider.Close()
	...
	startPath := "/v1/auth/oauth/github-app?mode=repository&workspace=" + c.WorkspaceID + "&next=" + url.QueryEscape("/app/conversations/"+c.ID+"?keep=yes")
	if w := request("GET", startPath, ""); w.Code != 303 || !strings.Contains(w.Header().Get("Location"), "session_expired") {
		t.Fatal("anonymous connection allowed")
	}
	start := request("GET", startPath, human)
	authURL, _ := url.Parse(start.Header().Get("Location"))
	if start.Code != 303 || authURL.Host != "github.com" || authURL.Query().Get("scope") != "" {
		t.Fatalf("start: %d %s", start.Code, start.Header().Get("Location"))
	}
	challenge = authURL.Query().Get("code_challenge")
	callbackPath := "/v1/auth/oauth/github-app/callback?code=authorization-code&state=" + authURL.Query().Get("state")
	callback := request("GET", callbackPath, human, start.Result().Cookies()...)
	if callback.Code != 303 || callback.Header().Get("Location") != "http://canter.test/app/conversations/"+c.ID+"?github=connected&keep=yes" {
		t.Fatalf("callback: %d %s", callback.Code, callback.Header().Get("Location"))
	}
