...
func (e *HTTPError) Error() string {
	return fmt.Sprintf("compute request failed with HTTP %d", e.StatusCode)
}

// TransportError keeps the underlying cause available to errors.Is/errors.As
// without exposing the operator-configured endpoint in logs or API responses.
// net/http wraps transport failures in url.Error, whose default string includes
...
func (e *TransportError) Error() string {
	operation := strings.ToLower(strings.TrimSpace(e.Operation))
	if operation == "" {
		operation = "request"
	}
	...
	return fmt.Sprintf("compute %s failed: %s", operation, reason)
}

func (e *TransportError) Unwrap() error { return e.Cause }

func privateTransportError(operation string, err error) error {
	if err == nil {
		return nil
	}
	...
	if errors.As(err, &requestErr) {
		if requestErr.Op != "" {
			operation = requestErr.Op
		}
		err = requestErr.Err
	}
	return &TransportError{Operation: operation, Cause: err}
}
...
func IsNetworkExhausted(err error) bool {
	var responseErr *HTTPError
	return errors.As(err, &responseErr) && strings.Contains(strings.ToLower(responseErr.Body), "no fixed ip addresses available")
}
...
func (c *Client) Delete(ctx context.Context, id string) error {
	...
	if err != nil {
		return privateTransportError(http.MethodDelete, err)
	}
...
func (c *Client) authenticate(ctx context.Context) (session, error) {
	...
	if err != nil {
		return session{}, privateTransportError(http.MethodPost, err)
	}
...
func (c *Client) request(ctx context.Context, method, url string, payload, target any) error {
	...
	if err != nil {
		return privateTransportError(method, err)
	}
	req.Header.Set("X-Auth-Token", s.Token)
	req.Header.Set("Content-Type", "application/json")
	resp, err := c.http.Do(req)
